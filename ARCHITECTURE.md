...
The application is "Theme-Agnostic". Light/Dark modes are handled automatically via raw RGB CSS variables in `index.css` mapped through `tailwind.config.js`.
...
### D. Markdown Typography (Cyberpunk/AI Vibe)
Markdown elements are styled via CSS variables in `index.css`. Do not hardcode colors in `MessageContent.tsx`.
- `--md-bold`: Cyan/Teal (`#2dd4bf`) for strong emphasis.
- `--md-italic`: Soft Purple (`#c084fc`) for internal thoughts/notes.
- `--md-h3`: Soft Coral (`#ff7a8a`) for headings.
- `--md-quote`: Amber/Gold (`#f59e0b`) for blockquotes.
