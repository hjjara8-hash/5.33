...
const CodeBlock: React.FC<React.PropsWithChildren<{ inline?: boolean; className?: string }>> = memo(({
  ...
  if (inline) {
    return (
      <code className="bg-bg-overlay/30 text-tint-indigo-text rounded px-1 py-0.5 font-mono text-sm border border-border-base">
        {children}
      </code>
    );
  }
...
const ReadModeView: React.FC<ReadModeViewProps> = memo(({ 
  ...
  return (
    <div
      className="fixed inset-0 bg-bg-panel/90 backdrop-blur-sm z-40 flex flex-col p-4 sm:p-8 md:p-12 pt-24 transition-colors duration-300"
      onClick={onClose} 
      role="dialog"
      aria-modal="true"
    >
      <div className="absolute top-4 right-4 z-10 flex items-center space-x-3">
          ...
          {handleReadModePrev && handleReadModeNext && (
              <div className="flex items-center bg-bg-element rounded-full p-1 mr-2 border border-border-base" dir="ltr">
                  <Button
                      variant="ghost"
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()} 
      >
        <div className={`bg-bg-panel text-text-primary border border-border-base p-6 sm:p-8 rounded-xl markdown-content shadow-2xl ${getFontSizeClass(readModeFontSizeLevel)} transition duration-200`}>
             <ReactMarkdown 
